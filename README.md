...
- **富文本图片自动上传（带上传后校验）**：steps/spec/verify 中 `<img>` 的本地路径或 data: URI 自动上传到禅道文件存储并替换为公开 URL，其余 HTML 逐字节保留。每次上传后匿名回读校验（200 + 字节一致），不一致自动刷新 token 重试（最多 3 次）——规避服务端"登录后首次上传静默存成 0 字节 .txt"导致的裂图；带附件的创建/变更会先做一次预热上传并在创建后核验附件实际字节数（返回 `attachmentsVerified`）
